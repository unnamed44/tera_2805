...
public final class Player extends Playable implements Nameable, Identified {
	...
	public boolean addSkill(Skill skill, boolean sendPacket) {
		...
		if(super.addSkill(skill, sendPacket)) {
			...
			if(sendPacket) {
				sendPacket(S_Skill_List.getInstance(this), true);
			}
	...
	public void deleteMe() {
		...
		synchronized(this) {
			...
			PlayerDetails2 detailsappearance = getdetails2();

			if(detailsappearance != null) {
				detailsappearance.fold();
				setdetails2(null, false);
			}
	...
	public PlayerDetails2 getdetails2()
	{
	...
	public void sendMessage(String message) {
		sendPacket(S_Chat.getInstance(Strings.EMPTY, message, SayType.SYSTEM_CHAT, 0, 0), true);
	}
	...
	public void setdetails2(PlayerDetails2 detailsappearance, boolean isNew) {
		this.detailsappearance = detailsappearance;
	}
	...
	public void teleToLocation(int continentId, float x, float y, float z, int heading) {
		...
		sendPacket(S_Load_Topo.getInstance(this), true);
	}
	...
	public void updateGuild() {
		...
		sendPacket(S_Guild_Info.getInstance(this), true);
