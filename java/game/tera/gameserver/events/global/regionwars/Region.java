...
public class Region extends SafeTask implements DieListener, PlayerSelectListener
{
	...
	public void onSelect(Player player)
	{
		...
		switch(getState())
		{
			...
			{
				player.sendPacket(S_Chat.getInstance(Strings.EMPTY, "Battle for \"" + getName() + "\" will start the " + nextBattleDate, SayType.NOTICE_CHAT, 0, 0), true);
			...
			case PREPARE_END_WAR:
			{
				...
				player.sendPacket(S_Chat.getInstance(Strings.EMPTY, "До окончания битвы за \"" + getName() + "\" осталось " + endTime + " мин.", SayType.NOTICE_CHAT, 0, 0), true);
				player.sendPacket(S_Chat.getInstance(Strings.EMPTY, "Контрольные точки:", SayType.NOTICE_CHAT, 0, 0), true);
				...
				for(int i = 0, length = control.length; i < length; i++)
				{
					...
					player.sendPacket(S_Chat.getInstance(Strings.EMPTY, spawn.getChatLoc() + "<FONT FACE=\"$ChatFont\" > [" + (owner == null? "NPC" : owner.getName()) + "]</FONT>", SayType.NOTICE_CHAT, 0, 0), true);
				}
	...
	{
		sendPacket(S_Chat.getInstance(Strings.EMPTY, message, SayType.NOTICE_CHAT, 0, 0));
	}
