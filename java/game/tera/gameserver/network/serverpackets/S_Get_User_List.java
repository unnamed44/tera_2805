 ...
 * @author Ronn
 */
public class S_Get_User_List extends ServerPacket
{
	private static final ServerPacket instance = new S_Get_User_List();

	public static S_Get_User_List getInstance(String accountName)
	{
		S_Get_User_List packet = (S_Get_User_List) instance.newInstance();
		...
		if(size > 0)
		{
			...
			for(int i = 0; i < size; i++)
			{
				...
				if(current == last) // last character sends 0x00
					...
					packet.writeShort(buffer, sizeTo + 401 + Strings.length(current.getName())); // need to update this from 283 to new
				...
				packet.writeInt(buffer, current.getHp()); // AD040000
				packet.writeInt(buffer, current.getMp()); // AD040000
				packet.writeInt(buffer, 0);// 1
				packet.writeInt(buffer, 0);// 2
				packet.writeInt(buffer, 0);// 7

				packet.writeInt(buffer, 0x00007E8F);//last online
				packet.writeByte(buffer, 0);
				packet.writeInt(buffer, 0);
				...
				PlayerDetails2 detailsappearance = current.getdetails2();
				...
				packet.writeLong(buffer, 0); //8
				packet.writeInt(buffer, 0); // 4
				packet.writeShort(buffer, 0); // 2

				packet.writeInt(buffer, 0); // BF1857B0
				...
				packet.writeLong(buffer, 0);
				packet.writeLong(buffer, 0);
				packet.writeLong(buffer, 0); //new
				packet.writeLong(buffer, 0); //new
				packet.writeLong(buffer, 0); //new
				packet.writeInt(buffer, 0); // new

				packet.writeInt(buffer, 0);//current rested xp?
				packet.writeInt(buffer, 0);//max restedxp ?
				...
				packet.writeByte(buffer, 1); // new

				packet.writeInt(buffer, 0);
				packet.writeInt(buffer, 0);//guild id
				packet.writeInt(buffer, 0);//unknow array
