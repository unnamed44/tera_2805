...
public class C_Whisper extends ClientPacket
{
	...
	protected void runImpl()
	{
		...
		player.sendPacket(S_Chat.getInstance(player.getName(), text, SayType.PRIVATE_CHAT, player.getObjectId(), player.getSubId()), true);
		enemy.sendPacket(S_Chat.getInstance(player.getName(), text, SayType.WHISHPER_CHAT, player.getObjectId(), player.getSubId()), true);
	}
}
